...
import { Wallet, DollarSign, TrendingUp, Target, Edit2, AlertCircle, Plus, Trash2, X, CheckSquare, FastForward, CheckCircle2 } from 'lucide-react';
...
const AnimatedNumber = ({ value }) => {
    ...
    useEffect(() => {
        ...
        const step = (timestamp) => {
            ...
            setDisplayValue(Math.floor(easeOut * (endValue - startValue) + startValue));
            if (progress < 1) {
...
const Dashboard = () => {
    ...
    const spendingPercentage = totalSpent === 0 ? 0 : (totalSpent / INITIAL_BUDGET) * 100;
    ...
    const handleAddExpense = (e) => {
        ...
        const expenseToAdd = {
            ...
            ...newExpense,
            amount: amount,
            categoryData: categories.find(c => c.name === newExpense.category)
        };
            ...
            <div className="bg-white border-b border-black/5 px-8 py-4 flex justify-between items-center sticky top-0 z-20">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500">Smart Budget Tracking</p>
                    </div>
                </div>
                <button 
            ...
            <main className="max-w-6xl mx-auto px-8 py-8 space-y-6">
                ...
                <div className="bg-white rounded-2xl p-6 border border-black/5 shadow-sm flex justify-between items-center group transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] hover:-translate-y-1 hover:shadow-md">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-sm font-medium text-slate-500">Working Professional Mode</p>
                            <h2 className="text-xl font-semibold text-slate-900">
                                Monthly Income: ₹<AnimatedNumber value={INCOME} />
                            </h2>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-black/5 shadow-sm transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] hover:-translate-y-1 hover:shadow-md">
                        ...
                        <h3 className="text-3xl font-bold text-slate-900 tracking-tight">
                            ₹<AnimatedNumber value={INITIAL_BUDGET} />
                        </h3>
                        <p className="text-xs text-slate-400 mt-2">Across 6 categories</p>
                    </div>

                    {/* Budget vs Spent */}
                    <div className="bg-white p-6 rounded-2xl border border-black/5 shadow-sm transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] hover:-translate-y-1 hover:shadow-md">
                        <div className="flex justify-between items-start mb-4">
                            <p className="text-sm font-medium text-slate-500">Budget vs Spent</p>
                            <div className="w-8 h-8 bg-emerald-50 text-emerald-600 rounded-lg flex items-center justify-center">
                                <DollarSign size={16} />
                            </div>
                        </div>
                        <h3 className="text-3xl font-bold text-slate-900 tracking-tight">
                            ₹<AnimatedNumber value={totalSpent} /> <span className="text-lg text-slate-400 font-medium">/ ₹{INITIAL_BUDGET.toLocaleString()}</span>
                        </h3>
                        ...
                        <p className="text-xs text-slate-400 mt-2">
                            {totalSpent > 0 ? `${spendingPercentage.toFixed(1)}% of budget used` : 'No expenses added yet'}
                        </p>
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-black/5 shadow-sm transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] hover:-translate-y-1 hover:shadow-md">
                        ...
                        <h3 className="text-3xl font-bold text-slate-900 tracking-tight">
                            ₹<AnimatedNumber value={INITIAL_BUDGET - totalSpent} />
                        </h3>
                        <p className="text-xs text-slate-400 mt-2">
                            {totalSpent > 0 ? 'All within budget' : 'Full budget available'}
                        </p>
                    </div>
                </div>
                     ...
                     <div className="bg-white p-6 rounded-2xl border border-black/5 shadow-sm transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] hover:-translate-y-1 hover:shadow-md group">
                        <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-2">
                                ...
                                <h3 className="font-semibold text-slate-900">Monthly Savings Goal</h3>
                            </div>
                        ...
                        <h4 className="text-2xl font-bold text-slate-900 tracking-tight mt-1 mb-2">
                             ₹<AnimatedNumber value={totalSpent > 0 ? INCOME - totalSpent : SAVINGS_GOAL} /> 
                             {totalSpent > 0 && <span className="text-sm font-medium text-slate-400 ml-1">of ₹{SAVINGS_GOAL.toLocaleString()}</span>}
                        </h4>
                        ...
                        {totalSpent > 0 ? (
                            <>
                                <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden mt-4">
                                     <div 
                                        className="h-full bg-emerald-500 rounded-full transition-all duration-700 ease-in-out"
                                        style={{ width: `${Math.min(((INCOME - totalSpent)/SAVINGS_GOAL)* 100, 100)}%` }}
                                     />
                                </div>
                                <div className="flex justify-between text-xs text-emerald-600 font-medium mt-2">
                                     <span>{Math.round(((INCOME - totalSpent)/SAVINGS_GOAL)* 100)}% saved</span>
                                     <span className="text-slate-400">₹{(INCOME - totalSpent - SAVINGS_GOAL).toLocaleString()} over goal</span>
                                </div>
                                <div className="mt-4 bg-emerald-50 rounded-lg p-2 text-center text-emerald-700 text-xs font-semibold flex items-center justify-center gap-2">
                                    <span className="text-sm">🎉</span> Goal achieved! Great job!
                                </div>
                            </>
                        ) : (
                            <>
                                <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden mt-4" />
                                <p className="text-xs text-slate-400 mt-3 pt-2 border-t border-slate-100">Potential savings based on your income</p>
                            </>
                        )}
                     </div>
                     ...
                     <div className="bg-white p-6 rounded-2xl border border-black/5 shadow-sm transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] hover:-translate-y-1 hover:shadow-md">
                        ...
                        {totalSpent === 0 ? (
                            <div className="flex items-center gap-4 mt-6">
                                ...
                                <div>
                                    <h4 className="text-lg font-semibold text-slate-600">Not yet tracked</h4>
                                    <p className="text-xs text-slate-400 mt-1">Add expenses to see your top spending category</p>
                                </div>
                            </div>
                        ) : (
                            <div className="flex items-start gap-4 mt-4">
                                ...
                                <div>
                                    <h4 className="text-sm font-medium text-slate-500 mb-1">{topCategory?.name || 'Food'}</h4>
                                    <div className="flex items-baseline gap-2">
                                        <span className="text-2xl font-bold text-orange-600">₹<AnimatedNumber value={topCategory?.spent || 0} /></span>
                                        <span className="text-xs font-medium text-slate-400">({Math.round(((topCategory?.spent || 0) / totalSpent) * 100 )}% of total)</span>
                                    </div>
                                </div>
                            </div>
                        )}
                ...
                {/* Smart Alerts */}
                <div>
                     <h3 className="text-sm font-semibold text-slate-900 mb-3">Smart Alerts</h3>
                     {totalSpent === 0 ? (
                        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex items-center justify-between animate-in fade-in zoom-in duration-500">
                           <div className="flex items-center gap-3 text-blue-700 text-sm font-medium">
                               <AlertCircle size={16} />
                               👋 Welcome! Start by adding your first expense to begin tracking your spending.
                           </div>
                        </div>
                     ) : (
                ...
                <div>
                     <h3 className="text-sm font-semibold text-slate-900 mb-3">Spending Overview</h3>
                     <div className="bg-white rounded-2xl border border-black/5 shadow-sm p-8 pb-4 relative overflow-hidden h-[300px]">
                         ...
                         <div className="absolute inset-0 px-12 py-8 pointer-events-none flex flex-col justify-between">
                            {[10000, 7500, 5000, 2500, 0].map((val, i) => (
                                <div key={i} className="flex items-center w-full">
                                    <span className="text-[10px] text-slate-400 w-8 pr-2 text-right">{val}-</span>
                         ...
                         <div className="absolute inset-0 px-12 pb-8 pt-8 flex items-end justify-between pl-20 pr-12">
                             {categories.map((cat, i) => {
                                 const budgetHeightPct = (cat.budget / 10000) * 100;
                                 const spentHeightPct = (cat.spent / 10000) * 100;
                                 
                                 return (
                                     <div key={cat.id} className="relative w-16 h-full flex items-end group">
                                         {/* Hover tooltip */}
                                         {totalSpent > 0 && (
                                            <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute -top-12 left-1/2 -translate-x-1/2 bg-white border border-slate-200 shadow-xl rounded-lg p-2 text-xs z-10 whitespace-nowrap pointer-events-none">
                                                <div className="font-semibold text-slate-700 mb-1">{cat.name}</div>
                                                <div className="flex justify-between gap-4">
                                                    <span className="text-blue-500">Spent: ₹{cat.spent}</span>
                                                </div>
                                                <div className="text-slate-400 mt-0.5">Budget: ₹{cat.budget}</div>
                                            </div>
                                         )}
                                         ...
                                         <div className="w-full relative flex items-end" style={{ height: '100%' }}>
                                            <div 
                                                ...
                                                style={{ height: `${budgetHeightPct}%`, zIndex: 1 }}
                                            />
                                            <div 
                                                ...
                                                style={{ 
                                                    height: totalSpent === 0 ? '0%' : `${spentHeightPct}%`, 
                                                    zIndex: 2,
                         ...
                         {totalSpent === 0 && (
                             <div className="absolute inset-0 bg-white/60 backdrop-blur-[1px] flex items-center justify-center z-10 animate-in fade-in duration-700">
                                 <div className="bg-white px-6 py-4 rounded-xl shadow-sm border border-slate-100 text-center flex flex-col items-center">
                                     ...
                                     <h4 className="font-semibold text-slate-700">No data yet</h4>
                                     <p className="text-xs text-slate-400 mt-1">Add expenses to see your spending trends</p>
                                 </div>
                             </div>
                         )}
                     ...
                     {totalSpent > 0 && (
                         <div className="flex justify-center items-center gap-4 mt-8 pb-4 text-xs font-medium text-slate-500">
                             <div className="flex items-center gap-2"><div className="w-3 h-3 bg-blue-500 rounded-sm"></div> Spent</div>
                             <div className="flex items-center gap-2"><div className="w-3 h-3 bg-slate-200 rounded-sm"></div> Budget</div>
                         </div>
                     )}
                </div>
                ...
                {/* Category Budgets */}
                <div>
                     <h3 className="text-sm font-semibold text-slate-900 mb-3 mt-6">Category Budgets</h3>
                     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                         {categories.map((cat) => {
                             ...
                             return (
                                 <div key={cat.id} className="bg-white rounded-xl border border-black/5 shadow-sm p-5 transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 group">
                                     ...
                                     <div className="flex items-baseline gap-1 mb-4">
                                         <span className="text-lg font-bold text-slate-900">₹<AnimatedNumber value={cat.spent} /></span>
                                         <span className="text-xs text-slate-500 font-medium">of ₹{cat.budget.toLocaleString()}</span>
                                     </div>
                                     ...
                                     <div className="flex justify-between items-center mt-2">
                                         <span className={`text-[10px] font-semibold ${isExceeded ? 'text-red-600' : 'text-emerald-600'}`}>
                                             {Math.round(usedPct)}% used
                                         </span>
                                         <span className="text-[10px] font-medium text-slate-400">
                                            ₹{Math.max(0, cat.budget - cat.spent).toLocaleString()} remaining
                                         </span>
                                     </div>
                                 </div>
                             );
                     ...
                     <div className="bg-white rounded-2xl border border-black/5 shadow-sm overflow-hidden min-h-[150px]">
                         {expenses.length === 0 ? (
                             <div className="flex flex-col items-center justify-center p-12 text-center h-full animate-in fade-in duration-500">
                                 <h4 className="text-sm font-semibold text-slate-600">No expenses recorded</h4>
                                 <p className="text-xs text-slate-400 mt-1 mb-4">Your expense history will appear here</p>
                                 <button 
                                    onClick={() => setIsModalOpen(true)}
                                     ...
                                     >
                                         <div className="flex flex-col">
                                             <div className="flex items-center gap-2">
                                                ...
                                                <span className="bg-blue-50 text-blue-600 text-[10px] font-semibold px-2 py-0.5 rounded-full">{expense.category}</span>
                                             </div>
                                             <span className="text-xs text-slate-400 mt-1 flex items-center gap-1">
                                                🗓️ {new Date(expense.date).toLocaleDateString()}
                                             </span>
                                         </div>
                                         <div className="flex items-center gap-4">
                            ...
                            <div className="space-y-1.5 relative">
                                ...
                                <div className="relative">
                                    ...
                                    <div className="absolute right-3 top-[3px] bottom-[3px] w-6 bg-slate-100 rounded-md flex flex-col pointer-events-none">
                                        <div className="h-1/2 flex items-end justify-center pb-0.5"><div className="w-0 h-0 border-l-[4px] border-r-[4px] border-b-[5px] border-transparent border-b-slate-600"></div></div>
                                        <div className="h-1/2 flex items-start justify-center pt-0.5"><div className="w-0 h-0 border-l-[4px] border-r-[4px] border-t-[5px] border-transparent border-t-slate-600"></div></div>
                                    </div>
                                </div>
                                {formErrors.amount && <p className="text-[10px] text-red-500 font-medium absolute -bottom-5">{formErrors.amount}</p>}
                            </div>
