...
export default function Onboarding() {
  ...
  const isStepValid = () => {
    switch (renderStep) {
      ...
      case 2: return income !== '' && Number(income) > 0;
      case 3: return budget !== '' && Number(budget) > 0;
      case 4: return category !== null;
      case 5: return savings !== '' && Number(savings) > 0;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0B1C2C] to-[#0F2A3F] flex flex-col items-center justify-center relative overflow-hidden font-sans">
      ...
      <div 
        ...
        style={{
          background: 'radial-gradient(circle at center, rgba(0,229,168,0.08), transparent 70%)',
          filter: 'blur(120px)'
        }}
      ></div>
      ...
      <header className="absolute top-0 w-full p-8 flex justify-center items-center z-10 select-none">
        ...
        <span className="text-white font-bold text-2xl tracking-wide">SpendWise</span>
      </header>
      ...
      <div className="w-full max-w-2xl px-6 z-10 flex flex-col items-center mt-12">
        ...
        {renderStep <= 5 && (
          <div className="w-full max-w-xl mb-12">
            <div className="flex justify-between items-center text-sm mb-3">
              <span className="text-[#94A3B8]">Step {renderStep} of 5</span>
              <span className="text-[#00E5A8] font-bold">{renderStep * 20}%</span>
            </div>
        ...
        <div className="w-full relative">
          <div className={`w-full ${isTransitioning ? 'step-exit' : 'step-enter'}`}>
            ...
            {renderStep === 2 && (
              <Card>
                ...
                <div className="mt-8">
                  <InputBox 
                    icon={<DollarSign className="w-5 h-5 text-[#94A3B8]" />}
                    placeholder="5000"
                    value={income}
            ...
            {renderStep === 3 && (
              <Card>
                ...
                <div className="mt-8">
                  <InputBox 
                    icon={<Wallet className="w-5 h-5 text-[#94A3B8]" />}
                    placeholder="3000"
                    value={budget}
            ...
            {renderStep === 4 && (
              <Card>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8">
                  <OptionBox icon="🍔" label="Food" compact selected={category === 'food'} onClick={() => setCategory('food')} />
                  <OptionBox icon="🛍️" label="Shopping" compact selected={category === 'shopping'} onClick={() => setCategory('shopping')} />
                  <OptionBox icon="✈️" label="Travel" compact selected={category === 'travel'} onClick={() => setCategory('travel')} />
                  <OptionBox icon="🎮" label="Entertainment" compact selected={category === 'entertainment'} onClick={() => setCategory('entertainment')} />
                </div>
              </Card>
            )}
            ...
            {renderStep === 5 && (
              <Card>
                ...
                <div className="mt-8">
                  <InputBox 
                    icon={<Target className="w-5 h-5 text-[#94A3B8]" />}
                    placeholder="1000"
                    value={savings}
            ...
            {renderStep === 6 && (
              <AnalyzingScreen navigate={navigate} />
            )}
...
function StepHeader({ title }) {
  return (
    <div>
      <p className="text-[#00E5A8] text-sm font-medium mb-3 tracking-wide">Let's personalize your experience</p>
      <h1 className="text-white font-bold text-3xl sm:text-4xl leading-tight">{title}</h1>
    </div>
  );
}
...
function OptionBox({ icon, label, selected, onClick, compact }) {
  return (
    <button
      ...
      `}
    >
      <span className={`${compact ? 'text-3xl' : 'text-5xl'} mb-3 drop-shadow-lg`}>{icon}</span>
      <span className="text-white font-medium text-sm sm:text-base">{label}</span>
    </button>
  );
}

function InputBox({ icon, placeholder, value, onChange, subtitle, hasSpinners }) {
      ...
      >
        ...
        {hasSpinners && (
          <div className="flex flex-col ml-3 shrink-0 text-[#94A3B8]">
            <button className="hover:text-white transition-colors" onClick={() => onChange({target: {value: String(Number(value || 0) + 100)}})}>▲</button>
            <button className="hover:text-white transition-colors mt-1" onClick={() => onChange({target: {value: Math.max(0, Number(value || 0) - 100)}})}>▼</button>
          </div>
        )}
      </div>
      <p className="text-[#94A3B8] text-xs sm:text-sm mt-4 font-medium">{subtitle}</p>
    </div>
  );
}

function AnalyzingScreen({navigate}) {
  ...
  return (
    <div className="w-full max-w-[600px] mx-auto bg-[#0f2a3f]/50 backdrop-blur-[20px] border border-white/5 rounded-[24px] p-10 sm:p-14 shadow-[0_20px_60px_rgba(0,0,0,0.5),_0_0_40px_rgba(0,229,168,0.05)] flex flex-col items-center justify-center min-h-[400px]">
      ...
      {/* Headings */}
      <h2 className="text-white font-bold text-2xl mb-2 text-center">Analyzing your data...</h2>
      <p className="text-[#94A3B8] text-sm mb-10 text-center">Preparing your personalized dashboard...</p>
      ...
      {/* Final Text */}
      <div className={`mt-10 transition-all duration-700 transform ${progress === 4 ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-4 scale-95'}`}>
...
function SequenceItem({ show, completed, text }) {
  return (
    <li className={`flex items-center space-x-4 transition-all duration-500 ease-out transform ${show ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
      <div className="w-6 h-6 flex items-center justify-center shrink-0">
        <div className={`transition-all duration-300 w-full h-full rounded-full flex items-center justify-center ${completed ? 'bg-[#00E5A8]/20' : 'border border-white/20'}`}>
          {completed && <CheckCircle className="text-[#00E5A8] w-4 h-4 animate-[checkPop_300ms_ease-out_forwards]" />}
        </div>
      </div>
      <span className={`text-sm font-medium transition-colors duration-300 ${completed ? 'text-white' : 'text-[#94A3B8]'}`}>{text}</span>
    </li>
  );
}
